...
if($action == "search"){
  ...
  search($link, $link, $tID, $locationID);
  $data["success"] = true;
  }
  else
  {
...
function search($link, $empID, $location){
  ...
  $stmt = $link->prepare("SELECT * FROM task WHERE locationID=$location AND taskStatus='Open'");
  $jobList= array();
  if($stmt->execute()) {
      ...
      if ($result->num_rows > 0) {
          while($row = $result->fetch_assoc()) {
              ...
              $stmt1 = $link->prepare("SELECT *  FROM taskHandlerSkills 
              WHERE taskHandlerID = $empID AND taskSkillsStatus = 1");
              if($stmt1->execute()) {
